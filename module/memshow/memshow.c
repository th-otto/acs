...
static AUSERBLK USERBLK01 = { A_3Dbutton, 0x29019178L, Auo_string, NULL, NULL, NULL, NULL, NULL };
	...
	if (self != NULL)
	{
		...
		for (i = 0; i < 4; i++)
		{
			...
			if (sizes[i] > 0)
			{
				...
				Aob_printf(self->work, BLOCK1 + i, "%8ld", sizes[i] >> 10);
				total += sizes[i];
			} else
			{
				ptrs[i] = NULL;
		...
		for (i = 3; i >= 0; i--)
			if (ptrs[i] != NULL)
				Mfree(ptrs[i]);
		Aob_printf(self->work, TOTAL, "%8ld", total >> 10);
