...
static AUSERBLK vslider = { A_wislider, 3L, Auo_wislider, NULL, NULL, NULL, NULL, NULL };
...
static int16 pman_keys(Awindow *self, int16 kstate, int16 key)
{
	...
	if (key < 0) /* key & NKF_FUNC */
	{
		...
		case NK_DOWN:
			Auo_editor(editor, AUO_EDGETROWS, &rows);
			if (sel.row1 < 0 || sel.row1 == rows - 1)
			{
				sel.row1 = sel.row2 = sel.col1 = 0;
				sel.col2 = 127;
			} else
			{
				sel.row1++;
