	...
	while ((char) key != 0)
	{
		if (key < 0) /* key & NKF_FUNC */
		{
			...
			case NK_M_PGUP:
				...
				{
					boolean line = ((char) key == NK_UP && !(key & NKF_SHIFT)) ||
						 ((char) key == NK_M_PGUP && (key & NKF_SHIFT) != 0);
					if (line)
			...
			case NK_M_PGDN:
				...
				{
					boolean line = ((char) key == NK_DOWN && !(key & NKF_SHIFT)) ||
						 ((char) key == NK_M_PGDN && (key & NKF_SHIFT) != 0);
					if (line)
			...
			case NK_M_END:
				{
					boolean line = ((char) key == NK_CLRHOME && !(key & NKF_SHIFT)) ||
					               ((char) key == NK_M_END && (key & NKF_SHIFT));
					serv(editor, AUO_EDUNSELECT, NULL);
					if (line)
					{
