...
static void out_string(Obj_Head *list, Obj_Head *map, const char *title)
{
	...
	if (count > 0)
	{
		...
		for (i = 0; i < count; i++)
		{
			...
			{
				info_obj(item->label); /* FIXME: should be mapitem */
				sprintf(iostring, "%s: packed array [1..%ld] of char = ", mapitem->label, item->used);
				save_string(iostring);
		...
		} else
		{
			...
			*dot = '\0';
			...
			for (i = 0; i <= MAX_LANGS; i++)
			{
				...
				{
					sprintf(iostring, "(*$IFDEF ACS_LANG%d*)(*$IFDEF ACS_LANG%d*)" NL, i + 1, j + 1);
					save_string(iostring);
					save_string("\t(*$ERROR \"More than one language macros defined!\"*)" NL "(*$ENDIF*)(*$ENDIF*)" NL);
				}
			}
			save_string(NL);
			for (j = 0; j <= MAX_LANGS; j++)
			{
				sprintf(iostring, "(*$IFNDEF ACS_LANG%d*)", j + 1);
				save_string(iostring);
			}
			sprintf(iostring, NL "\t(*$DEFINE ACS_LANG%d*)" NL, acs->language + 1);
			save_string(iostring);
			for (j = 0; j <= MAX_LANGS; j++)
				save_string("(*$ENDIF*)");
			...
			save_string(NL "(*$IFDEF ACS_LANG1*)\t\t\t(*\t\tAlerts: ACS_LANG1\t\t*)" NL NL);
			...
			for (j = 0; j <= MAX_LANGS; j++)
			{
				...
				if (j < MAX_LANGS)
				{
					sprintf(iostring, NL "(*$ELSE*)(*$IFDEF ACS_LANG%d*)\t\t\t(*\t\tAlerts: ACS_LANG%d\t\t*)" NL NL, j + 2, j + 2);
					save_string(iostring);
				}
			}
			save_string(NL "(*$ELSE*)" NL NL "\t(*$ERROR \"Wrong Language Macro defined\"*)" NL);
			for (j = 0; j <= MAX_LANGS; j++)
				save_string(NL "(*$ENDIF*)" NL);
			
			save_string(NL NL "(*\t\tSTRINGS\t\t*)" NL NL);
			save_string(NL "(*$IFDEF ACS_LANG1 *)\t\t\t\t\t(*\t\tStrings: ACS_LANG1\t\t*)" NL NL);
			for (j = 0; j <= MAX_LANGS; j++)
			{
				if (j < acs->language)
				{
				...
				if (j < MAX_LANGS)
				{
					sprintf(iostring, NL "(*$ELSE*)(*$IFDEF ACS_LANG%d*)\t\t\t(*\t\tStrings: ACS_LANG%d\t\t*)" NL NL, j + 2, j + 2);
					save_string(iostring);
				}
			}
